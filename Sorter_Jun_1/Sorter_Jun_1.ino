    ...
    if (ContExp)
    {
      ...
      if (entries[check] < maxEntries && TimeoutBool[check] == 1)
      {
        ...
        while (Mscanned == false && checkTimer < checkTime1)
        {
          ...
          if (mouseZone2 != mouseZone1 && mouseZone2 != "")
          { // the case where mouse that triggersgate to open turns around and another mouse enters the tube these conditions are set
            Mscanned = true;
            M2eM1 = false;
          }
          ...
          if (MouseInCenter)
          {
            ...
            if (Zone2Cont)
            {
              ...
              while (!MaxTestTimeReached && tORf)
              {
                ...
                if (mouseZone3 != "")
                { //********Changed bc mouse was able to move past gate
              ...
              while (tORf)
              {
                ...
                { ///// ADDED A TIME HERE AS THE FIX WHEN A MOUSE IS TRYING TO EXIT AND ANOTHER IS PRESENT IN TUBE 1
            ...
            else
            {
              ...
              while (k < 2 && rfidTimer < rfidTime)
              {
                ...
                if (readstring == mouseZone2 || readstring == mouseCheck)
                { ///////NEEDS TO BE FIXED MAKE mousecheck AN ARRAY BECAUSE MOUSECHECK COULD BE EQUAL TO MOUSEZONE 2
