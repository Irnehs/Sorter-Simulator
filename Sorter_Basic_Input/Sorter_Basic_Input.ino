...
void setup() {
  ...
  while(1) {
    Serial1.println("Hello world from RFID");
    delay(1000);
    Serial.println("Hello world from Sorter");
    delay(1000);
  }
  return;
  Serial1.write("SRA\r");
...
AllowEntry:  //This is a goto point when M @ A2 is not read, but there is a mouse in the center tube
              ...
              while (!MaxTestTimeReached && tORf) {
                ...
                if (mouseZone3 != "") {  //********Changed bc mouse was able to move past gate
                  ...
                  bool CorrectMouseInCenter = false;
                  //Wait until mouse triggers Antenna 2
                  while (Serial1.available() == 0 && CorrectMouseInCenter == false) {}
